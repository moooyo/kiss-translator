...
describe("Translator rule styles", () => {
  ...
  test("changing manual targets removes old translations and discovers new targets", async () => {
    ...
    await flushAsync();
    expect(
      document.querySelector(`#a .${Translator.KISS_CLASS.warpper}`)
    ).not.toBeNull();
  ...
  test.each([false, true])(
    ...
    async (enableBeforeIdle) => {
      ...
      await flushAsync();
      expect(
        document.querySelector(`#pending .${Translator.KISS_CLASS.warpper}`)
      ).not.toBeNull();
      ...
      expect(
        document.querySelector(`#later .${Translator.KISS_CLASS.warpper}`)
      ).not.toBeNull();
    }
  );
