...
import * as storageModule from "../libs/storage";
...
describe("subscription cache persistence", () => {
  ...
  test("observes cache updates from other settings views", async () => {
    await act(async () => {
      await storage.patchObj(STOKEY_SYNC, {
        dataCaches: { "other-view": 456 },
      });
    });
  ...
  test("does not replace a subscribed update with a stale pending read", async () => {
    const staleSync = await getSync();
    let finishRead;
    let markReadStarted;
    ...
    const read = jest
      .spyOn(storageModule, "getSyncWithDefault")
    ...
    try {
      await act(async () => {
        const pending = cache.reloadSync();
        await readStarted;
        await storage.patchObj(STOKEY_SYNC, {
          dataCaches: { "newer-source": 456 },
        });
        finishRead(staleSync);
        await pending;
      });
      expect(JSON.parse(container.textContent)).toEqual({
        existing: 123,
  ...
  test("tracks URL updates and deletion without changing unrelated field revisions", async () => {
    await act(async () => {
      await storage.patchObj(STOKEY_SYNC, { syncUser: "updated-user" });
    });
    const previousRevisions = await storage.getFieldRevisions(STOKEY_SYNC);
    const userPath = JSON.stringify(["syncUser"]);
    const cachePath = JSON.stringify(["dataCaches", "new-source"]);

    await act(async () => cache.updateDataCache("new-source"));
    const updatedRevisions = await storage.getFieldRevisions(STOKEY_SYNC);
    expect(updatedRevisions[cachePath]?.[0]).toBe(1);
    expect(updatedRevisions[userPath]).toEqual(previousRevisions[userPath]);

    await act(async () => cache.deleteDataCache("new-source"));
    const deletedRevisions = await storage.getFieldRevisions(STOKEY_SYNC);
    expect(deletedRevisions[cachePath]?.[0]).toBe(2);
    expect(deletedRevisions[userPath]).toEqual(previousRevisions[userPath]);
    expect((await getSync()).dataCaches).toEqual({ existing: 123 });
    expect((await getSync()).syncUser).toBe("updated-user");
  });
