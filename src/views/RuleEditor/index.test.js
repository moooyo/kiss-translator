...
jest.mock("../../libs/storage", () => ({
  storage: { getObj: jest.fn(), setObj: jest.fn(), patchObj: jest.fn() },
}));
...
beforeEach(() => {
  ...
  storage.patchObj.mockReset().mockResolvedValue();
...
test("both panels restore saved positions and persist keyboard movement", async () => {
  ...
  expect(storage.patchObj).toHaveBeenCalledWith(
    STOKEY_RULE_EDITOR_POSITION,
    { x: 184 },
    expect.objectContaining({ onWillWrite: expect.any(Function) })
  );
  expect(storage.patchObj).toHaveBeenCalledWith(
    STOKEY_RULE_INSPECTOR_POSITION,
    { x: 56 },
