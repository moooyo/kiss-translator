...
- Treat `moooyo/kiss-translator` (`origin`) as the default writable repository and the default pull request destination.
...
- **`gh` resolves to `upstream` by default in this checkout.** `gh repo view` returns
  `fishjar/kiss-translator`, so any `gh` invocation without an explicit `-R moooyo/kiss-translator`
