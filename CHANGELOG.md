...
- 统一设置页、Popup、划词翻译与字幕控件的 M3 圆角、状态层和焦点反馈，修复搜索框聚焦边框、窄屏密码标签及触屏悬停样式异常。
- 修复网页悬浮翻译按钮在点击展开过程中短暂变形、尺寸跳动或图标显色错误的问题，并稳定菜单、划词按钮及其 Shadow DOM 主题样式。
- 完整修复 Playground、提示词、字幕断句和划词翻译文本框的原生拉伸手柄裁切与遮挡问题，确保调整行高时可见且可操作。
- 修复快速切换文本、语言、翻译开关或订阅规则时旧异步结果覆盖新状态、重复触发及取消后继续写入的问题。
- 改进菜单、标签页、字幕列表和查词弹窗的键盘操作、焦点恢复、加载播报与禁用状态，补齐相关回归测试。

## v0.0.4

- 修复 Playground、提示词、字幕断句和划词翻译中可拉伸文本框的右下角手柄被大圆角裁切或被操作按钮遮挡的问题。
- 网页悬浮翻译按钮改为 M3 标准的 56×56 尺寸与 16px 固定圆角；悬停、聚焦和展开时不再突然变圆，贴边露出动画也更平滑。
- 移除悬浮按钮的整体透明度衰减，浅色与深色模式下均保持清晰的不透明底色，并使用对应的 M3 状态层反馈交互。
- 修复划词按钮逃出 Shadow DOM 后丢失主题样式、在滚动条或页面变换下越界的问题；同时统一设置页、Popup、字幕菜单和输入框浮动按钮的圆角与状态过渡。

## v0.0.3

- 修复查词提示框里的收藏与关闭按钮**从来点不到**：它显示在播放器右上角、字幕在底部中间，够到它要跨半个播放器，而此前离开单词 100ms 就收起。现在它是 popover，只有点关闭、点它外面、悬停另一个单词、字幕管理器销毁这四件事会关掉它，与时间无关。
- 提示框开着时视频保持暂停，关掉后才恢复；不再出现「伸手去点的路上视频自己跑起来」。
- 独立翻译窗口的出厂尺寸从 400×400 改为按内容计算，并在内容渲染后按实际高度收到刚好；首次打开居中于当前窗口。
- 独立翻译窗口最大化后内容居中并限宽，不再贴在左边、右侧留一大片空白。
- 移除赞赏与评价入口（三处：弹窗标题栏菜单、页脚展开区、设置加载失败态）——它们指向本项目没有的商店页与捐赠页。
- 「翻译此页」开关不再弹出重复的「已开启 / 已关闭」提示条，该提示原本会盖住划词翻译与暂停翻译两个按钮。
- 界面内的产品名改为 KISS Translator M3：此前只改了扩展列表显示名，应用自身渲染的标题仍是旧名。站点文案与 SEO 元数据一并更新。
