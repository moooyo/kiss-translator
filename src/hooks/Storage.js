...
export function useStorage(key, defaultVal = null, syncKey = "") {
  ...
  const applyPersistedSetting = useCallback(
    (storedValue) => {
      const previousPersisted = persistedSettingRef.current;
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (storedVal === undefined || storedVal === null) {
          if (isBackgroundManagedSetting) {
            // An empty patch asks the serialized background writer to persist
            // its default-backed current value without overwriting a setting
            // that another context may have initialized in the meantime.
